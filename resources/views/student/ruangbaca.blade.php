...
<body class="relative min-h-screen">
                ...
                <div id="ai-summary-result" class="mt-16 bg-[#75cb50]/5 dark:bg-[#75cb50]/10 border-2 border-[#75cb50]/30 rounded-3xl p-8 md:p-10 relative overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500 text-left">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 border-b border-[#75cb50]/20 pb-6">
                        ...
                        <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                            <button id="btn-download-pdf-ruangbaca" class="w-full sm:w-auto bg-[#75cb50]/10 hover:bg-[#75cb50]/20 text-[#75cb50] px-5 py-2.5 rounded-xl text-xs font-bold transition-all border border-[#75cb50]/30 active:scale-95 flex items-center justify-center gap-1.5">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div id="ai-summary-content-ruangbaca" class="prose prose-slate dark:prose-invert max-w-none text-slate-700 dark:text-slate-300 leading-relaxed text-lg">
            ...
            <div class="relative w-full max-w-4xl bg-white dark:bg-[#121212] border border-slate-200 dark:border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row md:h-[80vh] my-8">
                ...
                <div class="w-full md:w-2/5 p-6 md:p-8 bg-slate-50/50 dark:bg-white/[0.01] flex flex-col justify-between md:overflow-y-auto custom-scrollbar md:h-full text-left">
                    <div class="space-y-6">
                        ...
                        <button type="button" id="btn-download-pdf-modal" class="w-full mt-4 bg-slate-100 hover:bg-slate-200 dark:bg-white/5 dark:hover:bg-white/10 text-slate-800 dark:text-slate-200 py-3 rounded-xl text-xs font-bold transition-all active:scale-95 flex items-center justify-center gap-1.5 border border-slate-200 dark:border-white/10">
                            <svg class="w-4 h-4 text-[#75cb50]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                            </svg>
                            Download PDF
    ...
    <script>
        ...
        // PDF download function for Ruang Baca
        function downloadSummaryAsPDF(htmlContent, title = 'Rangkuman AI') {
            ...
            // Prevent elements from being split across page breaks
            doc.querySelectorAll('p, li, blockquote, h1, h2, h3, h4, h5, h6').forEach(el => {
                ...
                <div style="font-family: 'Inter', sans-serif; color: #1e293b; padding: 40px; background: #ffffff;">
                    <div style="border-bottom: 2px solid #75cb50; padding-bottom: 15px; margin-bottom: 30px; display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <div style="text-align: right; color: #64748b; font-size: 11px;">
                            ...
                            <p style="margin: 2px 0 0 0; font-weight: 500;">Materi: {{ $materi->judul_materi ?? 'Materi Pembelajaran' }}</p>
                        </div>
                    </div>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const btnRuangBaca = document.getElementById('btn-download-pdf-ruangbaca');
            if (btnRuangBaca) {
                btnRuangBaca.addEventListener('click', function() {
                    const contentDiv = document.getElementById('ai-summary-content-ruangbaca');
                    if (contentDiv) {
                        downloadSummaryAsPDF(contentDiv.innerHTML, 'Rangkuman AI: {{ $materi->judul_materi }}');
                    }
                });
            }

            const btnModal = document.getElementById('btn-download-pdf-modal');
            if (btnModal) {

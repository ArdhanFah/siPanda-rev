    ...
    <body class="relative min-h-screen">
        ...
        <div id="share-streak-modal" class="fixed inset-0 z-50 hidden items-center justify-center p-4 bg-slate-900/65 backdrop-blur-xl transition-opacity duration-300 opacity-0">
            <div class="glass max-w-3xl w-full p-6 sm:p-8 flex flex-col md:flex-row gap-6 md:gap-8 relative overflow-y-auto max-h-[90vh] bg-white/95 dark:bg-zinc-950/95 border border-white/20 dark:border-white/5 shadow-2xl animate-in fade-in zoom-in-95 duration-300">
                ...
                if ($streakTier == 1) {
                $cardBgStyle = "background: radial-gradient(circle at 0% 0%, rgba(117, 203, 80, 0.2), transparent 50%), radial-gradient(circle at 100% 100%, rgba(255, 140, 0, 0.2), transparent 50%), #121212;";
                $cardThemeColor = '#ff8c00';
                $cardTextClass = 'text-[#ff8c00]';
                $cardBorderClass = 'border-[#ff8c00]/30';
                } elseif ($streakTier == 2) {
                $cardBgStyle = "background: radial-gradient(circle at 0% 0%, rgba(117, 203, 80, 0.2), transparent 50%), radial-gradient(circle at 100% 100%, rgba(0, 198, 255, 0.2), transparent 50%), #121212;";
                $cardThemeColor = '#00c6ff';
                $cardTextClass = 'text-[#00c6ff]';
                $cardBorderClass = 'border-[#00c6ff]/30';
                } elseif ($streakTier == 3) {
                $cardBgStyle = "background: radial-gradient(circle at 0% 0%, rgba(117, 203, 80, 0.2), transparent 50%), radial-gradient(circle at 100% 100%, rgba(189, 0, 255, 0.2), transparent 50%), #121212;";
                $cardThemeColor = '#bd00ff';
                $cardTextClass = 'text-[#bd00ff]';
                $cardBorderClass = 'border-[#bd00ff]/30';
                } else {
                $cardBgStyle = "background: radial-gradient(circle at 0% 0%, rgba(117, 203, 80, 0.2), transparent 50%), radial-gradient(circle at 100% 100%, rgba(255, 215, 0, 0.25), transparent 50%), #121212;";
                ...
                <div class="flex-shrink-0 flex justify-center items-center mx-auto md:mx-0">
                    <!-- Share Card Element (Portrait Aspect Ratio for Social Media) -->
                    <div id="streak-share-card-element" class="relative w-[320px] h-[540px] rounded-[2rem] text-white overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10 select-none font-sans" style="{{ $cardBgStyle }}">

                        <!-- Top Pattern Overlay (siPanda identity) -->
                        <div class="absolute inset-0 opacity-10 bg-[url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSI4IiBoZWlnaHQ9IjgiPjxjaXJjbGUgY3g9IjMiIGN5PSIzIiByPSIxIiBmaWxsPSJyZ2JhKDI1NSwyNTUsMjU1LDEpIi8+PC9zdmc+')] pointer-events-none"></div>

                        <!-- Header -->
                        <div class="absolute top-0 left-0 right-0 h-[72px] px-6 flex items-center justify-between border-b border-white/5 z-10 bg-white/5">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl bg-white/10 border border-white/20 flex items-center justify-center overflow-hidden shadow-md">
                                ...
                                <div class="text-left">
                                    ...
                                    <p class="text-[8px] uppercase tracking-widest text-[#75cb50] font-black mt-1 leading-none">PBL Pembelajaran AI</p>
                                </div>
                            </div>
                            <div class="px-2.5 py-1 rounded-full bg-white/5 border {{ $cardBorderClass }} text-[8px] font-black {{ $cardTextClass }} tracking-wider shadow-inner">
                                KARTU RUNTUNAN
                            </div>
                        </div>

                        <!-- Center Body (Column Layout for Portrait) -->
                        <div class="absolute top-[72px] bottom-[72px] left-0 right-0 flex flex-col items-center justify-center gap-6 px-6 z-10">

                            <!-- Flame Container -->
                            <div class="relative w-[130px] h-[130px] rounded-full bg-white/[0.03] border border-white/10 flex items-center justify-center shadow-2xl">
                                <!-- Background Aura Glow -->
                                <div class="absolute inset-2 rounded-full opacity-40" style="background: radial-gradient(circle, {{ $cardThemeColor }} 0%, transparent 70%);"></div>

                                <!-- Glowing Flame SVG -->
                                <svg class="w-[80px] h-[80px] relative z-10" viewBox="0 0 24 24">
                                    <defs>
                            ...
                            <div class="flex flex-col items-center justify-center text-center">
                                <span class="text-[12px] font-black uppercase tracking-widest text-slate-400 mb-2">
                                    Berturut-turut
                                </span>
                                <div class="flex items-baseline justify-center gap-2 mb-4">
                                    <span class="text-[80px] font-heading font-black tracking-tight text-white leading-[0.85] drop-shadow-md">
                                        {{ $currentStreak }}
                                    </span>
                                    <span class="text-3xl font-bold {{ $cardTextClass }} opacity-90">
                                        Hari
                                    </span>
                                </div>
                                
                                <!-- Badge title -->
                                <div class="px-5 py-2 rounded-full bg-white/5 border border-white/10 text-xs font-bold text-slate-100 flex items-center justify-center gap-2 shadow-inner">
                                    <span class="w-2 h-2 rounded-full shadow-[0_0_8px_currentColor]" style="background-color: {{ $cardThemeColor }}; color: {{ $cardThemeColor }};"></span>
                                    {{ $streakTitle }}
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Footer -->
                        <div class="absolute bottom-0 left-0 right-0 h-[72px] px-6 flex items-center justify-between border-t border-white/5 z-10 bg-black/20">
                            <div class="text-left">
                                <div class="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Pelajar siPanda</div>
                                <div class="text-xs font-heading font-extrabold text-white truncate max-w-[130px]">
                                    {{ auth()->user()->name }}
                                </div>
                            </div>
                            <div class="flex flex-col items-end">
                                <div class="text-[10px] font-black uppercase tracking-widest text-[#75cb50]">#siPandaRuntunan</div>
                                <div class="text-[8px] text-slate-400 mt-1">sipanda.xhizoracodes.my.id</div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <script>
            ...
            function downloadShareCardImage() {
                ...
                // Remove rounded corners for the downloaded image as requested
                clone.style.borderRadius = '0';
                clone.classList.remove('rounded-[2rem]');
                ...
                document.fonts.ready.then(() => {
                    const options = {
                        ...
                        useCORS: true,
                        backgroundColor: '#121212', // Fallback background if transparent
                        logging: false,
                        onclone: function(clonedDoc) {
                            // Ensure the element in the iframe also has no border radius
                            const el = clonedDoc.getElementById('streak-share-card-element');
                            if (el) {
                                el.style.borderRadius = '0';
                                el.classList.remove('rounded-3xl', 'rounded-[2rem]');
                            }
                        }
                    };
